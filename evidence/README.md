...
The canonical `translation-decisions/` release expands those anchors into an exhaustive current occurrence concordance: 529 decisions and 26,087 exact source-target occurrences across 540 translated units containing a recorded decision. It includes full and priority review views, CSV, schema-valid JSON and the frozen cross-language schema. SyncTeX maps 2,897 occurrences in the 49-unit proof-systems reader to exact stable-PDF pages; all 23,190 occurrences outside that reader retain an explicit pending page field. `segment-canon-use.jsonl` is the complete current per-segment provenance record. Scope and QA files describe only the published or drafted tranche named in each file.

The current 554-unit checkpoint is bound by:
...
- `segment-canon-use.jsonl`: 1,440 one-to-one source/translation segment records, each naming canon passages actually consulted;
...
- `translation-decisions/`: the canonical expert-review package, including 529 decisions, 26,087 exact occurrences and 2,897 stable reader-page mappings;
- `qa-batches/`: all 246 current deterministic textual-QA receipts;
- `checkpoint-current.json`, `qa-current.json`, and `semantic-review-current.json`: the bounded 554/722 status and 648 semantic-review samples;
...
- `tex-set-theory-story-receipt.json`, `tex-set-theory-z-receipt.json`, and `tex-set-theory-ordinals-receipt.json`, with their corresponding render-QA files: the guarded builds, extraction/font checks and all-page visual review for the three set-theory readers through OLP-0557.
...
Canon originals are not redistributed here where their reuse license is unestablished. Their public URLs, exact local-byte hashes, locators, rendered-page hashes and evidence limitations remain in the ledgers so the consultation can be independently reconstructed. The tagged and checkpoint readers are explicitly interim editions and are not described as the complete 722-unit edition. Ten verified component readers cover 232 distinct units; 322 additional accepted units remain outside an all-page visually verified reader.
