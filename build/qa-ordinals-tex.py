from __future__ import annotations

from pathlib import Path
import hashlib
import json
import re
import subprocess


REPO = Path(__file__).resolve().parents[1]
STATE = Path(r"C:\interlanguage-task-state\openlogic-ta-Taml-IN")
ASSET = REPO / "release" / "openlogic-ta-Taml-IN-set-theory-ordinals-11-units.tex"
RECEIPT = STATE / "SET-THEORY-ORDINALS-TEX-SOURCE.json"
QA_PATH = STATE / "SET-THEORY-ORDINALS-TEX-SOURCE-QA.json"


def sha256(data: bytes) -> str:
    return hashlib.sha256(data).hexdigest()


receipt = json.loads(RECEIPT.read_text(encoding="utf-8"))
artifact = ASSET.read_bytes()
text = artifact.decode("utf-8")
assert len(artifact) == receipt["asset"]["bytes"]
assert sha256(artifact) == receipt["asset"]["sha256"]

block_re = re.compile(
    r"\\begin\{filecontents\*\}\{([^}]+)\}\n(.*?)\\end\{filecontents\*\}\n",
    re.DOTALL,
)
matches = list(block_re.finditer(text))
blocks = {match.group(1): match.group(2).encode("utf-8") for match in matches}
assert len(blocks) == len(matches) == 25
assert all("/" not in name and "\\" not in name and ".." not in name for name in blocks)

reader_rows = receipt["reader_units"]
assert len(reader_rows) == 11
assert {row["unit_id"] for row in reader_rows} == {
    f"OLP-{number:04d}" for number in range(547, 558)
}
verified_units = []
for row in reader_rows:
    name = f"olt-ord-unit-{row['unit_id']}.tex"
    assert name in blocks, name
    committed = subprocess.check_output(
        ["git", "show", f"{receipt['source_commit']}:{row['source_path']}"], cwd=REPO
    )
    assert blocks[name] == committed, row["unit_id"]
    assert sha256(committed) == row["translation_sha256"]
    verified_units.append(row["unit_id"])

required_embedded = {
    "olt-ord-open-logic.sty",
    "olt-ord-open-logic-locale.sty",
    "olt-ord-open-logic-referencing.sty",
    "olt-ord-open-logic-formulas.sty",
    "olt-ord-open-logic-tokenize.sty",
    "olt-ord-open-logic-selective.sty",
    "olt-ord-bussproofs-extra.sty",
    "olt-ord-ptolemaicastronomy.sty",
    "olt-ord-open-logic-config.sty",
    "olt-ord-open-logic-envs.sty",
    "olt-ord-tamil-config.sty",
    "olt-ord-tamil-pdf-math.sty",
    "olt-ord-open-logic.bib",
    "olt-ord-natbib-oup.bst",
}
assert required_embedded.issubset(blocks)

support_replacements = {
    r"\olpath/locale/\ollangid/open-logic-locale.sty": "olt-ord-open-logic-locale.sty",
    r"\olpath/open-logic-locale.sty": "olt-ord-open-logic-locale.sty",
    r"\olpath/sty/open-logic-referencing.sty": "olt-ord-open-logic-referencing.sty",
    r"\olpath/sty/open-logic-formulas.sty": "olt-ord-open-logic-formulas.sty",
    r"\olpath/sty/open-logic-tokenize.sty": "olt-ord-open-logic-tokenize.sty",
    r"\olpath/sty/open-logic-selective.sty": "olt-ord-open-logic-selective.sty",
    r"\olpath/sty/bussproofs-extra.sty": "olt-ord-bussproofs-extra.sty",
    r"\olpath/sty/ptolemaicastronomy.sty": "olt-ord-ptolemaicastronomy.sty",
    r"\olpath/open-logic-config.sty": "olt-ord-open-logic-config.sty",
    r"\olpath/locale/\ollangid/open-logic-envs.sty": "olt-ord-open-logic-envs.sty",
    r"\olpath/open-logic-envs.sty": "olt-ord-open-logic-envs.sty",
}
support_rows = receipt["support_files"]
assert len(support_rows) == len(required_embedded) == 14
for row in support_rows:
    source = subprocess.check_output(
        ["git", "show", f"{receipt['source_commit']}:{row['source_path']}"], cwd=REPO
    )
    assert sha256(source) == row["source_sha256"]
    expected = source
    if row["source_path"].endswith(".sty"):
        rewritten = source.decode("utf-8")
        for old, new in support_replacements.items():
            rewritten = rewritten.replace(old, new)
        expected = rewritten.encode("utf-8")
    assert sha256(expected) == row["embedded_sha256"]
    assert blocks[row["embedded_name"]] == expected, row["source_path"]

executable = text[text.rindex(r"\documentclass") :]
assert "../" not in executable and "..\\" not in executable
literal_inputs = re.findall(r"\\input\{([^}]+)\}", executable)
for target in literal_inputs:
    assert target in blocks, target
assert r"\bibliography{olt-ord-open-logic}" in executable
assert r"\bibliographystyle{olt-ord-natbib-oup}" in executable
assert executable.count(r"\TAIncludePath{content/set-theory/ordinals/ordinals.tex}") == 1
assert b"c:\\users" not in artifact.lower()
master = subprocess.check_output(
    ["git", "show", f"{receipt['source_commit']}:{receipt['reader_master']['source_path']}"],
    cwd=REPO,
)
assert sha256(master) == receipt["reader_master"]["source_sha256"]
assert sha256(executable.encode("utf-8")) == receipt["reader_master"]["assembled_executable_sha256"]

qa = {
    "schema": "openlogic-tamil-ordinals-tex-source-qa/1",
    "result": "pass",
    "source_commit": receipt["source_commit"],
    "asset": receipt["asset"],
    "scope": {
        "counted_reader_units": len(reader_rows),
        "exact_translation_blobs_verified": len(verified_units),
        "unaccepted_drafts_included": 0,
    },
    "embedded": {
        "filecontents_blocks": len(blocks),
        "project_support_files": len(required_embedded),
        "all_literal_executable_inputs_embedded": True,
        "bibliography_and_style_embedded": True,
    },
    "checks": {
        "asset_identity": True,
        "reader_unit_inventory": True,
        "translation_payloads_match_source_commit": True,
        "project_local_input_completeness": True,
        "support_payloads_match_source_commit_or_declared_path_rewrites": True,
        "reader_master_identity": True,
        "unaccepted_drafts_absent": True,
        "personal_path_absent": True,
        "accepted_pdf_replaced": False,
    },
}
QA_PATH.write_text(
    json.dumps(qa, ensure_ascii=False, indent=2) + "\n",
    encoding="utf-8",
    newline="\n",
)
print(json.dumps(qa, ensure_ascii=False, indent=2))
