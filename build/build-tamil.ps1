param([string]$Master='tamil-batch001.tex',[int]$Passes=2,[ValidateSet('xelatex','lualatex')][string]$Engine='xelatex',[ValidateRange(1000,60000)][int]$AcquisitionTimeoutMs=60000)
...
public class TamilTeXJob {
 ...
 public static uint Run(string exe,string args,string cwd) {
  ...
  } finally {if(assigned) {ACCT a;if(QueryInformationJobObject(job,1,out a,Marshal.SizeOf(typeof(ACCT)),IntPtr.Zero)&&a.active>0){TerminateJobObject(job,1);WaitForSingleObject(pi.process,10000);}}if(pi.thread!=IntPtr.Zero)CloseHandle(pi.thread);if(pi.process!=IntPtr.Zero)CloseHandle(pi.process);CloseHandle(job);}
 }
}
'@
 ...
 if($owned){
  foreach($extension in @('log','fls','aux','out')) {
 ...
 $receiptPath=Join-Path $state 'TEX-B001-RECEIPT.json'
 if(Test-Path -LiteralPath $receiptPath){Copy-Item -LiteralPath $receiptPath -Destination (Join-Path $state ('TEX-B001-RECEIPT-'+[DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss')+'.json'))}
